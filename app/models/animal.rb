...
class Animal < ActiveRecord::Base
  ...
  scope :parturition,   -> (current_user, date)   { joins('INNER JOIN "reproductions" ON "reproductions"."mother_id" = "animals"."id"').where("reproductions.insemination between ? and ? and reproductions.abortion IS NULL and reproductions.regress IS NULL and reproductions.parturition IS NULL", date.beginning_of_week - 274.days, date.end_of_week - 274.days).where(discard: nil, user_id: current_user.id) }
