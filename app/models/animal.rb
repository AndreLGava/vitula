...
class Animal < ActiveRecord::Base
  ...
  scope :parturition,   -> (current_user, date)   { joins('INNER JOIN "reproductions" ON "reproductions"."mother_id" = "animals"."id"').where("reproductions.insemination between ? and ? and reproductions.abortion IS NULL and reproductions.regress IS NULL and reproductions.parturition IS NULL", date - 2.days - 274.days, date + 7.days - 274.days).where(discard: nil, user_id: current_user.id) }
  ...
  scope :stop,          -> (current_user, date)   { joins('INNER JOIN "reproductions" ON "reproductions"."mother_id" = "animals"."id"').where("reproductions.insemination between ? and ? and reproductions.abortion IS NULL and reproductions.regress IS NULL", date - 5.days - 214.days, date + 7.days - 214.days).where(discard: nil, user_id: current_user.id) }
