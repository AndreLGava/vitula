class FunctionalitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @functionality.save
        format.html { redirect_to functionalities_path, notice: I18n.t('crud.saved') }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to functionalities_url, notice: I18n.t('crud.destroyed') }
  ...
  private
    ...
    def functionality_params
      params.require(:functionality).permit(:name, :description, :modulo_id)
    end
end
